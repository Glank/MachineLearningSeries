...
\AtBeginEnvironment{subsection}{\setcounter{solution}{0}}
...
as more than the apocalyptic fever dream James Cameron, and ran into a problem.
The existing notation for taking derivatives of matrix equations is so
deliberately obtuse as to suggest that some hidden cabal of mathematicians is
actively working to sabotage our glorious yet-to-be-built robot overlords.
That sort of justice cannot stand! 

I thus set out to invent a system of my own. Much to my surprise, some of the
results with were rather elegant. For example, I stumbled upon a
...
parts. I knew I had succeeded when I was able to derive the derivative of the softmax
function completely in my own notation (Thm. \ref{softmax_derivative}).

Now, not knowing what else to do with the results, I've written this paper
(short book?) about my amature exploration into multidimensional matricies -
...
$\partial B_{r_1, c_1}/\partial X_{r_2, c_2}$ is defined by the rate of change of
...
and my current toolkit does not serve that need!
...
We'll say that for the whole number vector $\vec{v}$ which describes the shape of
...
Multiplication of traditional matricies is one of those things that, when one first
learn about it, causes one to wonder ``By which satanic cult was this process
proscribed?''
And then, after seeing it used in practice, one slowly comes to understand the
reason behind the madness. That is, that madness is the natural state of things
and matrix multiplication is used to punish those few kind souls who dare to
...
\newpage
\begin{example}
Let $|A| = \left<2, 3, 2\right>$ and $|B| = \left<3, 2, 2\right>$ where
the elements of $A$ and $B$ are defined in the tables below.
\begin{table}[h!]
\begin{center}
...
\subsection*{Section 1 Solutions}
